...
Intègre le téléchargement automatique des sous-fichiers "custom" de categories.txt
"""
...
if __name__ == "__main__":
    ...
    # 2. Fichiers de contenu de base
    print("\n[2/3] Téléchargement des fichiers de contenu principaux…")
    ...
    # 3. Récupération et analyse automatique des sous-fichiers
    print("\n[3/3] Détection et décryptage automatique des sous-fichiers…")
    
    # --- A. Analyse des catégories et décryptage automatique des liens "custom" ---
    cats_data = results.get("categories.txt")
    if isinstance(cats_data, list):
        print("\n  📁 Recherche de catégories de type 'custom'...")
        custom_count = 0
        for item in cats_data:
            if not isinstance(item, dict):
                continue
            cat_raw_str = item.get("cat", "")
            if not cat_raw_str:
                continue
                
            # Décoder la chaîne JSON imbriquée
            try:
                cat_obj = json.loads(cat_raw_str) if isinstance(cat_raw, str) else cat_raw
            except Exception:
                try:
            ...
            # Si le type de la catégorie est "custom", on télécharge son fichier de chaînes
            if cat_obj.get("type") == "custom":
                custom_api_path = cat_obj.get("api", "")
                if custom_path and custom_path.endswith(".txt"):
                    print(f"     [Détecté] Catégorie '{cat_obj.get('name')}' ➔ '{custom_path}'")
                    decoded_custom_channels = download(cfg, custom_path)
                    if decoded_custom_channels is not None:
                        save(custom_path, decoded_custom_custom)
                        

    # --- B. Analyse des Événements et de leurs sous-fichiers de liens ---
    events = results.get("events.txt")
    if isinstance(events, list):
        print("\n  📂 Recherche des sous-fichiers de liens des événements sportifs…")
        fetched = 0
        for ev in events:
            if not isinstance(ev, dict):
                continue
            
            # Les événements peuvent aussi contenir un JSON imbriqué dans la clé "event"
            event_raw = ev.get("event", "")
            try:
                event_obj = json.loads(event_raw) if isinstance(event_raw, str) else event_raw
            except:
                event_obj = ev
            ...
            if links_path and links_path.endswith(".txt"):
                d = download(cfg, links_path)
                if d is not None:
                    save(links_path, d)
                    fetched += 1
                if fetched >= 10:  # Limite à 10 fichiers pour éviter de surcharger (ajustable)
                    print("    (limité à 10 fichiers d'événements pour le test)")
                    break

    print(f"\n{'=' * 65}")
    print(f"  Tous les fichiers décryptés sont sauvegardés dans : ./{OUT_DIR}/")
    print("=" * 65)